...
`pyproject.toml` sets `testpaths = ["tests"]`, so normal discovery runs eleven test modules under `tests/`. V11-01 increased discovery from 24 to 77 tests, V11-02 to 107, V11-03/V11-04 to 203, V11-05 to 289, V11-07 to 365, V11-R to 379, and V11-07R to 401:
...
- `tests/test_input_capabilities.py`: 98 V11-07/V11-07R cases for naming-hint placeholders, success-only completion celebration, the images/PDF/CSV UI policy, retained core compatibility, independent image/native PDF/PDF OCR states, unsupported and unavailable selection boundaries, picker/folder/drop routes, stale/failed worker probes, and run preflight. Runtime readiness is mocked; OCR qualification is not repeated. Run independently with `python -m pytest -q tests/test_input_capabilities.py`.
