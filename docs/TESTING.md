...
- `tests/test_version_identity.py`: 15 V11-R and PRODUCT-D2-I1 cases for the canonical `core._version.__version__` source, the `pyproject.toml` dynamic-version contract, the runtime banner and UI version surfaces, installed distribution metadata, the repository and package version verifiers (including divergence detection and the local-wheel DnD direct reference), and release-documentation invariants (CHANGELOG candidate section, protected release-process integration path, OCR-qualification reconciliation, and ROADMAP completed-unit/next-admission/deferred markers). Run independently with `python -m pytest -q tests/test_version_identity.py`.
...
The canonical workload, fixture, environment, measurement, correctness, repeatability, evidence-retention, and optimization-admission contracts are in [Reproducible performance baseline](PERFORMANCE_BASELINE.md). Benchmark outputs do not replace any required test or GitHub check.

Validate the harness without collecting an authoritative baseline:
