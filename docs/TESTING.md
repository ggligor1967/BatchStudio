...
- `tests/test_performance_harness.py`: 19 V12-PERF cases for deterministic fixtures, hash tampering, workload/configuration rejection, bounded timeouts, failure propagation, real B1 execution and collision ownership, relative worker roots, statistics, worker identity, metadata, calibration evidence, exact session comparison, committed-evidence recomputation, and no-overwrite evidence writes. Run independently with `python -m pytest -q tests/test_performance_harness.py`.
