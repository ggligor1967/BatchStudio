...
- `tests/test_performance_harness.py`: V12-PERF cases for deterministic fixtures, hash tampering, workload/configuration rejection, bounded timeouts, failure propagation, real B1 execution and collision ownership, statistics, worker identity, metadata, calibration evidence, exact session comparison, and no-overwrite evidence writes. Run independently with `python -m pytest -q tests/test_performance_harness.py`.
