...
The compiler starts with type `any`. A per-file operation that returns a concrete `output_type` advances the current type; `same` preserves it. A valid compiled result declares `per_file` or `aggregate` execution and lists every enabled operation ID in order. An aggregate result additionally records `original_inputs` as its input source and the aggregate's accepted registry types. Invalid compilation returns errors without an executable plan.

Only one aggregate operation is registered: `pdf_merge`. Contract A requires it to be the **only enabled step**. Disabled predecessors or successors do not participate in compilation or execution. Enabled per-file predecessors are rejected with an instruction to disable or remove the other steps; transformed intermediates are not composed into aggregate execution and runtime never falls back from a declared transformation to the original input. The per-file worker rejects an aggregate declaration explicitly instead of skipping it.
...
After workflow validation and compilation, an empty aggregate batch records one batch-level `No input files` error before output-directory preparation, path allocation, or `begin`; timing and running state settle normally. This also applies to aggregate dry run. For a nonempty batch, core and UI preflight validate original input paths and registry types against the compiled `pdf` requirement. Any wrong or mixed type rejects the whole batch before output preparation or `begin`, with the operation, expected type, actual type, and input name in the core error.

After successful preflight, the processor creates a fresh `PDFAggregateMergeOperation`, resolves and reserves one contained `.pdf` destination, calls `begin`, then calls `consume` on the original input paths in selection order. Each readable PDF contributes all pages in its original order to one writer. Correctly typed but unreadable PDFs are recorded as per-input failures; readable PDFs can still be merged under the preserved V11-02 partial-invalid-input policy.
