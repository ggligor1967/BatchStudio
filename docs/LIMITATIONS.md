...
- XLS, XLSX, TXT, JSON, and XML remain Level 1 generic core compatibility formats. They are not product-admitted inputs, and no registered operation parses or transforms their content.
- The programmatic `file_rename` path copies those opaque files byte-for-byte under a new name. It provides no format-aware fidelity guarantee. OCR-generated TXT and workflow/settings JSON are output/control contracts, not TXT or JSON input processing.
...
- PDF watermark rendering is structurally qualified only for the documented valid CropBox and cardinal-rotation geometry matrix. It supports the existing centered text watermark, Helvetica, 45-degree angle, gray/opacity style, zero margin, and shrink-only containment; it does not provide arbitrary anchors, graphic overlays, typography layout, or pixel-identical output across renderers. See [Operations](OPERATIONS.md#pdf-watermark-geometry-contract).
...
- Dry run uses typed symbolic intermediates without fabricating files. Generated-content checks are explicitly deferred; conditional plans are assessed plans, not completed transformations. A demonstrated failure rejects the chain, and unsupported planning never invokes execution. Aggregate-only dry run retains its original-input boundary. See [ADR-0005](adr/0005-multistep-dry-run-planning.md).
- Final names/suffixes are reserved before operation writes; occupied destinations cause alternate batch allocation or explicit exclusive-creation failure. Direct operation calls are protected too. HTML/CSV processing reports do not allocate alternate names because their API returns no path; they fail without changing an occupied caller-selected destination. Settings/workflow persistence retains its separate replacement semantics.
