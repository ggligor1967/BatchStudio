...
The boundary is not a filesystem sandbox. It does not defend against arbitrary hostile concurrent directory/link replacement or guarantee recovery from every OS/filesystem failure. Dry-run validation is read-only and creates neither a directory nor a probe; it does not prove future normal-run write permission. Its execution/report boundary is defined in the [security model](../SECURITY_MODEL.md#dry-run-output-suppression). Aggregate compilation and stop/finalize lifecycle behavior and OCR extraction semantics are unchanged.
