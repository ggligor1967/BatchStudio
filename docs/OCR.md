...
V11-05 verification uses deterministic mocked executables, languages, rasterization, and recognition; it establishes the capability and contract behavior described above, not real recognition accuracy. Controlled real-OCR qualification is defined by V11-06 (issue #10) as a separate, conditional unit: it was **not admitted to the 1.1.0 release scope and is deferred** as future work in [Roadmap](ROADMAP.md). Real OCR success is therefore still not verified in this repository, and 1.1.0 ships the deterministic mocked contract coverage only. No OCR binaries or language data were installed for this work, and OCR production behavior is unchanged. See [Testing](TESTING.md#ocr-tests) and the historical [v1.0.0 verification record](releases/v1.0.0-verification.md).
