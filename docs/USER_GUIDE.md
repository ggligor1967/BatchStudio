...
Picker filters come from the backend input types and the current workflow's operation requirements. Without a workflow, all ingestible formats remain selectable: image editing and file rename do not require OCR. With a workflow, unavailable input paths are omitted. Files selected through the dialog, folder scan, or drop hook are checked again before acceptance; unsupported inputs and missing prerequisites receive a specific refusal. Readiness checks run in a worker, and a changed workflow requires a fresh selection.
