...
All registered per-file writers, aggregate PDF finalization, and normal HTML/CSV report writers use `exclusive_output`: exclusive creation (`x`/`xb`) followed by writes through the created handle. An occupied destination, including one created after reservation, causes an explicit failure without truncating or replacing it. Direct operation calls receive the same write protection; batch calls can allocate an alternative final path. Successful operation results identify the actual destination. Report generation returns `False` on collision instead of selecting an unreportable alternate name, and only a successfully generated current-run report is eligible for the HTML viewer. Dry-run report export is rejected.
...
Both operation bases retain `supports_dry_run`. Per-file planning additionally requires explicit `supports_planning`; it never falls back to `execute`. Typed descriptors distinguish actual sources from planned destinations. Existing source validators remain active, while generated-content and future ownership checks are deferred. In-memory name allocation creates no on-disk reservation. A new normal run repeats source, capability, destination, probe, and ownership checks. [ADR-0005](adr/0005-multistep-dry-run-planning.md) defines staged validation and immutable run context with separately isolated mutable reservations. [Planning regressions](../tests/test_planning_contracts.py) also intercept global temporary-file and transformation attempts. [Write-interception regressions](../tests/test_dry_run_contracts.py) detect attempted writes through output-scoped primitives as well as checking actual directory contents before and after. OCR capability paths are deterministic mocks; no real OCR success is claimed.
