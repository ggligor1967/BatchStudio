...
V12-PERF (issue #37) established deterministic F1-F5 fixtures and hashes, frozen B1-B5 workloads, exact environment and dependency identity, warm measurement and variance policies, fail-closed correctness guardrails, retained raw samples, two-session repeatability evidence, and noise-derived regression budgets. Both sessions passed at source commit `05ad00dc8f5b29e5fb6a0f1dcfe7828c5a3c59d2`; bounded profiling established no actionable bottleneck, so no optimization unit is admitted. The unit changed no production runtime behavior; see [Reproducible performance baseline](PERFORMANCE_BASELINE.md).
