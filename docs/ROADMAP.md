...
- **Dependencies:** BACKLOG-H0 and the V11-01 output-ownership foundation. It follows V11-08 in the governance order but has no technical dependency on GUI coverage.
...
- **Dependencies:** BACKLOG-H0, the V11-02 aggregate lifecycle, and V12-01 final-output ownership. It precedes V11-06 so later qualification runs use settled workflow semantics.
...
- **Dependencies:** BACKLOG-H0, completed V11-07 capability-truth work, V11-08 behavioral coverage, and settled V12-02 workflow semantics. If support is selected, a new approved implementation unit and dependency decision are required.
...
- **Dependencies:** BACKLOG-H0 and stable output/workflow contracts from V12-01/V12-02. Contract and fixtures precede implementation; existing pypdf/reportlab behavior remains authoritative until then.
...
- **Dependencies:** BACKLOG-H0 and stable semantics from all preceding units. External OCR or PDF tools are benchmark inputs only when their exact versions and data are fixed.
