...
That is expected. Dry run creates no directory, probe, operation output, or automatic/manual report. Read-only path feasibility does not verify future write permission. Later steps receive symbolic artifacts: checks requiring generated bytes remain deferred, while demonstrated configuration, source, type, or naming failures reject the plan; see [Limitations](LIMITATIONS.md#dry-run-and-output-safety).
